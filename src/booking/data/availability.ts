import type { Booking, DateKey, Table } from './types.ts'
import { service } from './venue.ts'
import { addMinutes, allSlots, occupancyMinutes, serviceWindow, sittingFor } from './time.ts'
...
function withinService(start: Date, end: Date, date: DateKey): boolean {
  const [open, close] = serviceWindow(date)
